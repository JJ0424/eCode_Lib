...
void UartRxTimeProc(UartRunT *uart_dma)
{
    ...
    cnt = DMA_GetCurrDataCounter(UartDmaConst[idx].rx_channel); cnt = _UART_DMA_BUFFER_SIZE - cnt;

    // 'pass_time' ++
    if ((uart_dma->overtime.last_cnt == cnt) && (uart_dma->overtime.last_cnt != 0)) {

        uart_dma->overtime.pass_time++;
    }
